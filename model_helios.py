...
HELIOS_CACHE_STALE_SECONDS = 120
HELIOS_BG_REFRESH_INTERVAL = 30
HELIOS_BRAIN_FILE = os.path.join(DATA_DIR, 'model_brain', 'helios_brain.pkl')
HELIOS_MODEL_FILE = os.path.join(DATA_DIR, 'model_brain', 'helios_model.pkl')
HELIOS_MODEL_NAMES = ['LightGBM', 'XGBoost', 'ExtraTrees', 'LogisticRegression']
...
_AUTOTRAINER_LAST_RUN = 0
_AUTOTRAINER_DAILY_INTERVAL = 86400
...
# ───────────────────────────────────────────────────────────────────────────
#  HeliosLearner – Self-learning brain with 4 ML models
# ───────────────────────────────────────────────────────────────────────────
...
class HeliosLearner:
    def __init__(self):
        self.models = {
            ...
            'ExtraTrees':        {'wins':0,'losses':0,'total':0,'acc':50.0,'recent_wins':0,'recent_losses':0,'recent_acc':50.0,'consecutive_losses':0,'consecutive_wins':0},
            'LogisticRegression':{'wins':0,'losses':0,'total':0,'acc':50.0,'recent_wins':0,'recent_losses':0,'recent_acc':50.0,'consecutive_losses':0,'consecutive_wins':0},
        }
        self.weights = {
            'LightGBM': 0.35, 'XGBoost': 0.28, 'ExtraTrees': 0.20, 'LogisticRegression': 0.17,
        }
    ...
    def _adjust_weights(self):
        ...
        if total_acc > 0 and len(accs) >= 2:
            for name in self.weights:
                self.weights[name] = max(0.05, min(0.55, accs[name]/total_acc * 3.0))
...
def _get_learner():
    ...
    with _learner_lock:
        if _learner is not None:
            return _learner
        try:
            os.makedirs(os.path.dirname(HELIOS_BRAIN_FILE), exist_ok=True)
            if os.path.exists(HELIOS_BRAIN_FILE):
                with open(HELIOS_BRAIN_FILE, 'rb') as f:
                    _learner = pickle.load(f)
                return _learner
        except Exception:
            pass
        _learner = HeliosLearner()
        return _learner
...
def _save_learner():
    ...
    try:
        os.makedirs(os.path.dirname(HELIOS_BRAIN_FILE), exist_ok=True)
        tmp = HELIOS_BRAIN_FILE + '.tmp'
        with open(tmp, 'wb') as f:
            pickle.dump(_learner, f)
        os.replace(tmp, HELIOS_BRAIN_FILE)
    except Exception:
        pass

_HELIOS_MODEL_BRAIN_DIR = os.path.join(DATA_DIR, 'model_brain', 'helios_models')

def _save_model_brain(model_name, data):
    try:
        os.makedirs(_HELIOS_MODEL_BRAIN_DIR, exist_ok=True)
        path = os.path.join(_HELIOS_MODEL_BRAIN_DIR, f'{model_name}.pkl')
        tmp = path + '.tmp'
        with open(tmp, 'wb') as f:
            pickle.dump(data, f)
        os.replace(tmp, path)
...
def _load_model_brains():
    brains = {}
    if not os.path.isdir(_HELIOS_MODEL_BRAIN_DIR):
        return brains
    for fname in os.listdir(_HELIOS_MODEL_BRAIN_DIR):
        if fname.endswith('.pkl'):
            model_name = fname[:-4]
            try:
                with open(os.path.join(_HELIOS_MODEL_BRAIN_DIR, fname), 'rb') as f:
...
def _predict_extratrees(training_rows, current_slice, daily_history=None):
    ...
    try:
        ...
        model = ExtraTreesClassifier(
            ...
            min_samples_split=5, random_state=48, n_jobs=1,
...
def _predict_logistic(training_rows, current_slice, daily_history=None):
    if not SKLEARN_AVAILABLE or SklearnLogisticRegression is None:
        return None
    try:
        X, y, _ = _build_training_data(training_rows)
        ...
        model = SklearnLogisticRegression(
            ...
            max_iter=500, random_state=49, class_weight='balanced',
        )
        model.fit(X, y)
        proba = model.predict_proba(feats.reshape(1, -1))[0]
        big_prob = proba[1] if len(proba) > 1 else proba[0]
        pred = 'BIG' if big_prob >= 0.5 else 'SMALL'
        conf = 50 + abs(big_prob - 0.5) * 100
        return {'prediction': pred, 'confidence': round(min(conf, 90), 2), 'bigProbability': round(big_prob * 100, 2)}
    except Exception:
        return None
...
def _model_loss_manager(learner, training_rows, model_predictions, big_votes, small_votes):
    ...
    if actual_counts['BIG'] > actual_counts['SMALL']:
        recovery_side = 'BIG'; reason = 'loss_actual_majority'
    elif actual_counts['SMALL'] > actual_counts['BIG']:
        recovery_side = 'SMALL'; reason = 'loss_actual_majority'
    elif pred_counts['BIG'] > pred_counts['SMALL']:
        recovery_side = 'SMALL'; reason = 'opposite_failed_big'
    elif pred_counts['SMALL'] > pred_counts['BIG']:
        recovery_side = 'BIG'; reason = 'opposite_failed_small'
    elif regime == 'ZIGZAG' and recent_actuals:
        recovery_side = 'SMALL' if recent_actuals[0] == 'BIG' else 'BIG'; reason = 'zigzag_recovery'
    elif regime == 'STREAK' and recent_actuals:
        recovery_side = recent_actuals[0]; reason = 'streak_recovery'
    else:
        recovery_side = model_vote_side; reason = 'model_vote_recovery'
    ...
    return {
        **signal, 'active': True, 'prediction': recovery_side, 'reason': reason,
        'boost': round(min(boost, 0.56), 4), 'confidence': round(min(94, 68 + len(losses) * 4), 2),
        'lossPredictions': pred_counts, 'lossActuals': actual_counts,
        'regime': regime, 'streakLen': streak_len, 'zigzagCount': zigzag_count,
    }
...
def _autotrainer():
    global _AUTOTRAINER_LAST_RUN
    now = time.time()
    if now - _AUTOTRAINER_LAST_RUN < _AUTOTRAINER_DAILY_INTERVAL:
        return
    _AUTOTRAINER_LAST_RUN = now
    try:
        all_preds = load_daily_1k_history(limit=None)
        ...
        print(f'[HELIOS_AUTOTRAINER] Error: {e}')
...
def _learning_source_summary(rows=None):
    ...
    return {
        ...
        'displayHistoryLimit': HELIOS_HISTORY_LIMIT,
...
def _ensure_files():
    os.makedirs(os.path.dirname(HELIOS_HISTORY_CSV), exist_ok=True)
    os.makedirs(os.path.dirname(HELIOS_BRAIN_FILE), exist_ok=True)
    if not os.path.exists(HELIOS_HISTORY_CSV):
        with open(HELIOS_HISTORY_CSV, 'w', newline='') as f:
...
def _bootstrap_from_daily():
    ...
    with _memory_entries_lock:
        if _memory_entries:
            return
    csv_has_data = False
    if os.path.exists(HELIOS_HISTORY_CSV):
        try:
            with open(HELIOS_HISTORY_CSV, 'r', newline='') as f:
...
def _verify_memory_entries():
    ...
    with _memory_entries_lock:
        ...
        for entry in _memory_entries.values():
            ...
            if entry.get('prediction') in ('BIG','SMALL'):
                pattern_name = entry.get('patternUsed') or entry.get('patternused') or 'helios_ensemble'
...
def _start_verify_loop():
    ...
    with _verify_thread_lock:
        if _verify_thread and _verify_thread.is_alive():
            return
        t = threading.Thread(target=_verify_loop, daemon=True, name='helios_verify')
        _verify_thread = t
        t.start()
...
def _model_from_pattern(pattern):
    ...
    if 'lgbm' in p or 'lightgbm' in p or 'lgb' in p:
        ...
        return 'XGBoost'
    elif 'extra' in p or 'extratrees' in p or 'et' in p:
        return 'ExtraTrees'
    elif 'logistic' in p or 'lr' in p:
...
# ─── Core Prediction: 4-Model Weighted Ensemble ──────────────────────────
...
def _predict(learner, training_rows, current_slice, daily_history):
    ...
    predictors = [
        ...
        ('ExtraTrees', _predict_extratrees),
        ('LogisticRegression', _predict_logistic),
    ]
    ...
    for name, predictor_fn in predictors:
        try:
            ...
            if result and result.get('prediction') in ('BIG', 'SMALL'):
                w = learner.weights.get(name, 0.20)
                ...
                model_predictions.append({
                    'model': name, 'prediction': result['prediction'],
                    'confidence': round(result['confidence'], 2),
                    'probability': result['bigProbability'],
                    'weight': round(w, 4), 'available': True,
                })
            else:
                model_predictions.append({
                    'model': name, 'prediction': None, 'confidence': 0,
                    'probability': 50, 'weight': 0, 'available': False,
                })
        except Exception:
            model_predictions.append({
                'model': name, 'prediction': None, 'confidence': 0,
    ...
    if not total_weight or total_weight == 0:
        ...
        if ml_result and ml_result.get('prediction') in ('BIG', 'SMALL'):
            ...
            model_predictions.append({
                'model': 'EnsembleFallback', 'prediction': ml_result['prediction'],
                'confidence': conf, 'probability': float(ml_result.get('bigProbability', 50)),
    ...
    if recovery['active'] and recovery['side']:
        ...
        model_predictions.append({
            'model': 'LossRecovery', 'prediction': recovery['side'],
            'confidence': round(55 + recovery['boost'], 2),
            'probability': 50 + recovery['boost'],
            'weight': round(boost / 100, 4), 'available': True,
        })
    ...
    if loss_manager['active'] and loss_manager['prediction'] in ('BIG', 'SMALL'):
        ...
        model_predictions.append({
            'model': 'DeepLossManager', 'prediction': loss_manager['prediction'],
            'confidence': loss_manager['confidence'], 'probability': 50,
            'weight': round(loss_manager['boost'], 4), 'available': True,
        })
    ...
    for mp in model_predictions:
        ...
        if hist_acc > 0:
            model_ranking.append({
                'model': mn, 'historicalAccuracy': hist_acc,
    ...
    return {
        'prediction': pred, 'confidence': round(confidence, 2),
        ...
        'regime': regime, 'streakLen': streak_len, 'zigzagCount': zigzag_count,
        'bestModel': {
            'name': best_model_name, 'historicalAccuracy': round(best_model_acc, 2),
            'confidence': best_model_conf, 'prediction': best_model_pred,
...
def _public_entry(row):
    return {
        'period': row.get('period'), 'prediction': row.get('prediction'),
        'status': row.get('status', 'Pending'),
        'confidence': float(row.get('confidence') or 0),
        'actual': row.get('actual'), 'number': row.get('number'),
        'patternUsed': row.get('patternUsed') or row.get('patternused') or '',
        ...
        try:
            ...
            if should_predict:
                try:
                    ...
                    print(f'[HELIOS_PREDICT] error: {e}')
            ...
            print(f'[HELIOS] Payload computed: {current.get("prediction")} @ {current.get("confidence")}%')
        except Exception as e:
            print(f'[HELIOS_COMPUTE] error: {e}\n{traceback.format_exc()}')
            try:
...
def get_helios_payload():
    ...
    if _payload_cache:
        ...
        c['stale'] = True
        c['staleReason'] = 'helios_refresh_in_progress'
        if 'warming' in c:
            del c['warming']
            del c['warmingReason']
        _bg_refresh()
...
def _save_cache(payload):
    try:
        ...
        tmp = HELIOS_CACHE_FILE + '.tmp'
        with open(tmp, 'w', encoding='utf-8') as f:
            json.dump({'timestamp': time.time(), 'payload': _convert_native(payload)}, f)
        os.replace(tmp, HELIOS_CACHE_FILE)
    except Exception as e:
        print(f'[HELIOS_CACHE] save error: {e}')

def _load_cache():
    if not os.path.exists(HELIOS_CACHE_FILE):
        return None, None
    try:
...
def get_cached_helios_payload():
    ...
    if age > HELIOS_CACHE_STALE_SECONDS:
        result['stale'] = True
...
def _bg_refresh():
    ...
    with _bg_lock:
        if _bg_thread and _bg_thread.is_alive():
            return
        t = threading.Thread(target=_bg_worker_with_timeout, daemon=True, name='helios_bg')
        _bg_thread = t
        t.start()
...
def _bg_worker_with_timeout():
    try:
        ...
        print('[HELIOS_BG] worker timed out')
    ...
    def _loop():
        while True:
            try:
                ...
                print('[HELIOS_BG] compute timed out, skipping this cycle')
            except Exception as e:
                print(f'[HELIOS_BG] loop error: {e}')
            time.sleep(HELIOS_BG_REFRESH_INTERVAL)
    t = threading.Thread(target=_loop, daemon=True, name='helios_bg_loop')
    t.start()
    print('[HELIOS_BG] Background refresh + verify loop started')
